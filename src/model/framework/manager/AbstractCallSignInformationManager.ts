...
export abstract class AbstractCallSignInformationManager<T extends Merged>
    extends AbstractRequestEngine
    implements ISearchBase<SearchType>
{
    ...
    public getPartialAtcClientInfo(
        partialCallSign: string
    ): (IvaoAtc | VatsimAtis)[] {
        const clients = this.isIvaoInfo(this._info)
            ? this._info.clients.atcs
            : this._info.controllers;
    ...
    private buildSearchIndex(): void {
        ...
        const { pilots } = this.isIvaoInfo(this._info)
            ? this._info.clients
            : this._info;
        const atcs = this.isIvaoInfo(this._info)
            ? this._info.clients.atcs
            : this._info.controllers;
    ...
    public getClientInfo(
        callSign: string,
        type: "pilot" | "atc"
    ): IvaoPilot | IvaoAtc | VatsimPilot | VatsimAtc {
        let retVal: IvaoPilot | IvaoAtc | VatsimPilot | VatsimAtc;
        const clients = this.isIvaoInfo(this._info)
            ? this._info.clients
            : this._info;
        if (type === "pilot") {
            retVal = (clients.pilots as (IvaoPilot | VatsimPilot)[]).find(
                (pilot) => pilot.callsign === callSign
            );
        } else {
            const atcs: (IvaoAtc | VatsimAtc)[] = this.isIvaoInfo(this._info)
                ? this._info.clients.atcs
                : this._info.controllers;
    ...
    public search(
        ...
        const filteredSearch = search.filter(
            (resultItem) => resultItem.item.type === selectedType
        );
