                ...
                if (aircraft[1] && aircraft[1].trim() === callsign) {
                    return {
                        ...
                        baro_altitude: aircraft[7]
                            ? `${(aircraft[7] * 3.28084).toFixed(0)} ft`
                            : "On Ground",
                        on_ground: aircraft[8] ? aircraft[8] : "Unknown",
                        velocity: aircraft[9]
                            ? `${(aircraft[9] * 1.943844).toFixed(0)} knots`
                            : "Parked",
                        true_track: aircraft[10]
                            ? `${aircraft[10].toFixed(0)}°`
                            : "Unknown",
                        vertical_rate:
                            aircraft[11] !== null
                                ? `${(aircraft[11] * 196.8504).toFixed(0)} fpm`
                                : "On Ground",
                        sensors: aircraft[12] ? aircraft[12] : "Unknown",
                        geo_altitude: aircraft[13]
                            ? `${(aircraft[13] * 3.28084).toFixed(0)} ft`
                            : "On Ground",
