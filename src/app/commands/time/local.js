...
module.exports = class LocalCommand extends (
  Command
) {
  constructor(client) {
    super(client, {
      ...
      memberName: 'local',
      description:
        'Gives you the specific local time for the given zulu Time and ICAO',
      args: [
        {
          key: 'icao',
          type: 'string',
        ...
        },
        {
          ...
          validate: (val) => {
            if (!val) return true;
            if (val.length !== 4) return 'Zulu time must be in HHMM format';
  ...
  async run(msg, { icao, zulutime }) {
    ...
    if (!(icao || zulutime)) {
      zuluEmbed
        .setColor('#ff0000')
        .setDescription('Command syntax: local [ICAO] [Zulu time]');
      ...
      try {
        ...
        zuluEmbed
          .setTitle(`Local Time at ${icao} when zulu time is ${zulutime}hrs`)
          .setDescription(`${timestring}hrs`);
      } catch (error) {
        zuluEmbed
          .setColor('#ff0000')
