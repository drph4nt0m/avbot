...
module.exports = class Avwx {
  ...
  static async download() {
    if (
      this.vatsim.general.UPDATE &&
      this.formatDate(this.vatsim.general.UPDATE)
        .add(4, 'minute')
        .isAfter(dayjs())
    ) {
      return;
    ...
    const [, general, clients, servers] = data
      .split(/!GENERAL:|!CLIENTS:|!SERVERS:|!PREFILE:/g)
      .map((r) => r.trim());
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        Object.keys(this.vatsim.clients).forEach((callSign) => {
          if (
            this.vatsim.clients[callSign].clientType === 'ATC' &&
